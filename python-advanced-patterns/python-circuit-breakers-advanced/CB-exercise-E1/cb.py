...
class CBListener(pybreaker.CircuitBreakerListener):
	def state_change(
		...
		logging.debug(f"CBListener.state_change({old_state}, ->   "
		              f"{new_state})")
...
@retry(stop = stop_after_attempt(3), wait = wait_fixed(3), before_sleep =
before_sleep_log(logging.getLogger(), logging.DEBUG))
@circuit_breaker
def call_service_b():
	logging.debug(f"call_service_b()")
	
	
	outcome = requests.get("http://127.0.0.1:5001/calling_service_b")
	outcome.raise_for_status()
	print(outcome.json())
	logging.info(f"[INFO]: In call_service_b()", "old state %s :: ->  current "
	                                             "state  ::  RESULT : %s ",
	             circuit_breaker.state, circuit_breaker.current_state,
	             outcome.text)
	return outcome.text

def safe_call_service_b():
	logging.debug(f"safe_call_service_b()")
	try:
