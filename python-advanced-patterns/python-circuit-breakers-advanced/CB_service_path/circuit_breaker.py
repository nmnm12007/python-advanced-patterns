...
class CBListener(pybreaker.CircuitBreakerListener):
    def state_change(self, cb, old_state, new_state):
        logging.warning(f"CBListener changed state from {old_state} to {new_state}")
...
def call_service_b():
    ...
    logging.info(
        "[CALL]: Service B :: http://127.0.0.1:5001/process :: CB State:  %s",
    ...
    response = requests.get("http://127.0.0.1:5001/process", timeout=2)
...
def safe_call():
    try:
        ...
        logging.error("[FAIL-FAST] Circuit OPEN → Service B NOT called")
        return "Fallback: Service B is down : Circuit is OPEN", 503

    except Exception as ex:
        logging.error(f"[DOWNSTREAM FAILURE] {ex}")
