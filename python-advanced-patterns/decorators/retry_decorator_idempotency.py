...
def log_success(func, result):
	...
	f_start = time.strftime("%Y-%m-%d %H:%M:%S",
	                        time.localtime(start_time))
	tmp_str = (
			f"[{f_start}] : [Success] : {func.__name__} with "
			f"is successful "
			f":  {result} \n")
	print(tmp_str)
...
def log_failure(func, attempt, retries, last_exception):
	start_time = time.time()
	f_start = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
			start_time))
	tmp_str = (f"[{f_start}] : [ERROR] : Attempt {attempt+1} on {func.__name__} failed with exception:"
	           f" {last_exception}, "
	           f" Attempts Failed \n")
...
def log_retry(func, attempt, retries, last_exception, time_delay):
	...
	f_start = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(
			start_time))
	tmp_str = (f"[{f_start}] : [WARNING] : Attempt {attempt+1} out of "
...
def retry(retries = 3, retry_on = (),
          base_delay = 0.5,
          idempotent = True, on_retry = log_retry, on_success = log_success,
			...
			for i in range(retries):
				try:
					...
					if idempotent:
						...
						on_retry(func, i, retries, last_exception,
						         time_delay)
					else:
