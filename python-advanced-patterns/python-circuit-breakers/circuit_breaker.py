...
@circuit_breaker
def call_service_b():
    logging.info(f"[CB STATE - BEFORE CALL] {circuit_breaker.current_state}")

    response = requests.get("http://localhost:5001/process", timeout=2)
    response.raise_for_status()

    logging.info(f"[CB STATE - AFTER CALL] {circuit_breaker.current_state}")
...
class CBListener(pybreaker.CircuitBreakerListener):
    def state_change(self, cb, old_state, new_state):
        logging.warning(f"🔥 CB TRANSITION: {old_state} → {new_state}")
